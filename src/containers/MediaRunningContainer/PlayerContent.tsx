...
const PlayerContent: FC<PlayerContentProps> = ({
  ...
  const post = currentMediaRunning.postData;
  ...
  const renderLeft = () => {
    ...
    return (
      <div
        className={`mr-2 flex items-center flex-grow lg:flex-shrink-0 lg:basis-48 overflow-hidden`}
      >
        <Link
          to={href}
          className="relative h-16 flex items-center w-full max-w-[240px] xl:flex-grow space-x-3 pl-12 "
        >
          <NcImage
  ...
  const renderDurationTime = () => {
    return (
      <div className="absolute w-full inset-x-0 bottom-full">
        ...
        ></div>
        <div
          className="absolute h-0.5 min-w-0 left-0 top-1/2 transform -translate-y-1/2 rounded-full bg-primary-500 will-change-contents"
